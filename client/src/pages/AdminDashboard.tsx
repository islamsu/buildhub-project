...
import {
  ...
  UserCheck, UserX, Save, RefreshCw, ClipboardCheck, FileSearch, RotateCcw, XCircle, SendHorizontal, Download, CalendarDays, Loader2, UserPlus, Trash2, History, Power,
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [accountDraft, setAccountDraft] = useState({ name: '', username: '', email: '', phone: '', userRole: 'homeowner', note: '' });
  ...
  const createDummyUser = trpc.admin.createDummyUser.useMutation({
    onSuccess: data => { toast.success(lang === 'ar' ? `تم إنشاء المستخدم التجريبي: ${data.username}` : `Dummy user created: ${data.username}`); setCreateAccountType(null); setAccountDraft({ name: '', username: '', email: '', phone: '', userRole: 'homeowner', note: '' }); utils.admin.users.invalidate(); },
    onError: error => toast.error(error.message),
  });
