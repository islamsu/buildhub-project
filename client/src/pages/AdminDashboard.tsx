...
export default function AdminDashboard() {
  ...
  const filteredRegistrationApplicants = useMemo(() => filterRegistrationApplicants(complianceQueue, { role: registrationRoleFilter, from: registrationDateFrom, to: registrationDateTo }).filter(applicant => {
    const query = registrationSearch.trim().toLowerCase();
    return !query || `${applicant.name ?? ''} ${applicant.email ?? ''}`.toLowerCase().includes(query);
  }), [complianceQueue, registrationRoleFilter, registrationDateFrom, registrationDateTo, registrationSearch]);
