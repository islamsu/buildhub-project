...
import {
  ...
  UserCheck, UserX, Save, RefreshCw, ClipboardCheck, FileSearch, RotateCcw, XCircle, SendHorizontal,
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const { data: complianceQueue = [], isLoading: complianceLoading } = trpc.admin.complianceQueue.useQuery(undefined, { enabled: isAdmin });
  ...
  const registrationSummary = useMemo(() => {
    const counts = summarizeComplianceRegistrations(complianceQueue, lang === 'ar').map(row => ({ role: row.label, pending: row.pending, approved: row.approved }));
      ...
      <div className="space-y-6" dir={dir}>
        ...
        <Card>
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div><CardTitle className="flex items-center gap-2"><ClipboardCheck className="h-5 w-5 text-primary" />{lang === 'ar' ? 'ملخص تسجيل المحترفين' : 'Professional registration summary'}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{lang === 'ar' ? 'مقارنة الطلبات قيد الانتظار والمعتمدة حسب الفئة.' : 'Compare pending and approved registrations by professional role.'}</p></div>
            <div className="flex flex-wrap gap-2 text-xs"><Badge className="border-amber-200 bg-amber-50 text-amber-700">{lang === 'ar' ? 'قيد الانتظار' : 'Pending'}: {registrationSummary.pending}</Badge><Badge className="border-emerald-200 bg-emerald-50 text-emerald-700">{lang === 'ar' ? 'معتمد' : 'Approved'}: {registrationSummary.approved}</Badge></div>
          </CardHeader>
          <CardContent><ResponsiveContainer width="100%" height={260}><BarChart data={registrationSummary.counts} layout="vertical" margin={{ top: 4, right: 12, left: 8, bottom: 4 }} barCategoryGap="22%"><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" allowDecimals={false} tick={{ fontSize: 11 }} /><YAxis type="category" dataKey="role" width={lang === 'ar' ? 110 : 125} tick={{ fontSize: 11 }} /><Tooltip cursor={{ fill: 'hsl(var(--muted) / 0.35)' }} /><Legend /><Bar dataKey="pending" name={lang === 'ar' ? 'قيد الانتظار' : 'Pending'} stackId="status" fill="#f59e0b" radius={[4, 0, 0, 4]} isAnimationActive={false} /><Bar dataKey="approved" name={lang === 'ar' ? 'معتمد' : 'Approved'} stackId="status" fill="#10b981" radius={[0, 4, 4, 0]} isAnimationActive={false} /></BarChart></ResponsiveContainer></CardContent>
        </Card>
