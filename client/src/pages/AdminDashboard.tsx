...
import AdminVendorEnquiries from '@/components/AdminVendorEnquiries';
...
function formatInvitationStatus(status: string | null | undefined, lang: 'en' | 'ar') {
  const labels: Record<string, string> = lang === 'ar'
    ? { none: 'لا توجد', invitation_sent: 'تم إرسال الدعوة', pending_setup: 'في انتظار الإعداد', password_set: 'تم تفعيل الحساب', expired: 'منتهية' }
    : { none: 'None', invitation_sent: 'Invitation Sent', pending_setup: 'Pending Setup', password_set: 'Account Activated', expired: 'Expired' };
  return status ? labels[status] ?? status : '—';
}
...
export default function AdminDashboard() {
  ...
  // derived from the pair, so the URL is too. It is what a notification links
  // to and what an administrator pastes into a ticket.
...
<button type="button" onClick={() => setSelectedGroup('all')} className={`rounded-lg border p-3 text-start transition-colors ${selectedGroup === 'all' ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'}`}><p className="text-xs text-muted-foreground">{lang === 'ar' ? 'الكل' : 'All Users'}</p><p className="text-lg font-semibold">{totalUserCount}</p></button>{ROLE_GROUPS.map(group => <button type="button" key={group.key} onClick={() => setSelectedGroup(group.key)} className={`rounded-lg border p-3 text-start transition-colors ${selectedGroup === group.key ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'}`}><p className="truncate text-xs text-muted-foreground">{lang === 'ar' ? group.ar : group.en}</p><p className="text-lg font-semibold">{groupCounts[group.key] ?? 0}</p></button>)}</div></CardHeader>
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                <label className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <select className="h-8 rounded-md border bg-background px-2 text-xs" value={userSort} onChange={event => { setUserSort(event.target.value as 'newest' | 'name' | 'role'); setUserPage(0); }}>
                    <option value="newest">{lang === 'ar' ? 'الأحدث' : 'Newest'}</option>
                    <option value="name">{lang === 'ar' ? 'الاسم' : 'Name'}</option>
                    <option value="role">{lang === 'ar' ? 'المجموعة' : 'Group'}</option>
                  </select>
                </label>
                <div className="flex items-center gap-2">
                  <Button type="button" size="sm" variant="outline" className="h-7" disabled={userPage === 0} onClick={() => setUserPage(page => page - 1)}>{lang === 'ar' ? 'السابق' : 'Previous'}</Button>
                  <span className="text-xs text-muted-foreground">{lang === 'ar' ? `صفحة ${userPage + 1} من ${userPageCount}` : `Page ${userPage + 1} of ${userPageCount}`}</span>
                  <Button type="button" size="sm" variant="outline" className="h-7" disabled={userPage + 1 >= userPageCount} onClick={() => setUserPage(page => page + 1)}>{lang === 'ar' ? 'التالي' : 'Next'}</Button>
                </div>
              </div>
              </CardContent>
            </Card>
          ...
          {/* The vendor-enquiry control plane. Its own section rather than a
              panel inside billing: an enquiry is a marketplace relationship,
          ...
          <TabsContent value="operations"><div className="space-y-6"><AdminDataQuality /><AdminOperationalHealth />{can('audit.read') && <AdminAuditTrail />}{can('marketplace.manage') && <AdminSponsorships />}{can('marketplace.manage') && <AdminFeaturedProviders />}{can('marketplace.manage') && <AdminFeaturedProducts />}</div></TabsContent>
