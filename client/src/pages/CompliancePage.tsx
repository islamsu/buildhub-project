...
export default function CompliancePage() {
  ...
  const handleFile = async (file?: File) => {
    ...
    try {
      ...
      await uploadDocument.mutateAsync({ documentType: selectedRequirement.type, fileName: file.name, contentType: file.type, base64 });
      setUploadProgress(100);
    } catch {
      // The mutation reports a localized error toast; reset the transient upload state below.
    } finally {
      setTimeout(() => { setUploadingName(''); setUploadProgress(0); setUploadSpeed(0); setUploadEta(null); setSelectedRequirement(null); }, 400);
