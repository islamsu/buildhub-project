...
export default function ProjectDetailEnhancements({ projectId, lang }: { projectId: number; lang: 'en' | 'ar' }) {
  ...
  /*
   * A PROJECT'S RECORD, REPORTED AS NEVER WRITTEN.
   *
   * Both lists defaulted to `[]`, so a failed request told everyone on the
   * project that no milestone had been set and no progress had ever been
   * reported - on the two tabs a client opens to find out exactly that.
   */
  const milestonesQuery = trpc.projects.milestones.useQuery({ projectId }, { retry: false });
  const milestones = milestonesQuery.data ?? [];
  const reportsQuery = trpc.projects.progressReports.useQuery({ projectId }, { retry: false });
