...
- [ ] ERROR-IS-NOT-EMPTY, remaining server-side DEBT (classified and enforced in `truthfulEmptyStates.test.ts`, cannot grow unnoticed): `analytics/events.ts` (a zeroed FUNNEL, same class as the revenue one), `billing/service.ts`, `billing/enquiries.ts:58`, `billing/lifecycle.ts:751`, `db.ts` user lookups returning undefined, `publicPlacement.ts`.
