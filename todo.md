...
- [ ] ERROR-IS-NOT-EMPTY: the remaining 18 surfaces on the debt list in `truthfulEmptyStates.test.ts` (AdminUserDetail, MessagesPage, ProductDetail, ProductFormPage, RFQRespondPage, AdminDataQuality, AdminEnquiryAllowance, AdminFeaturedProviders, AdminPlacements, AdminRfqInvestigation, AdminSponsorships, AdminVendorEnquiries, AdminVendorNameChanges, PortfolioManager, ProjectDetailEnhancements, VendorNameChangeRequest, VendorReputation, Home). Each renders an empty state and never looks at the error. The list is enforced: it cannot grow, and an entry that gets fixed must be removed from it.
