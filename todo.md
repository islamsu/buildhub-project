...
- [x] A NAME IN THE ADMIN CONSOLE IS A WAY INTO THE RECORD — nineteen admin surfaces render a person or a business by name; SIX made it a link and THIRTEEN printed it as text, so an administrator reading a support ticket, a sponsorship, an audit line, a reported review or a product listing had the name in front of them and no way to act on it. The six that DID link had each written the link themselves, in four spellings, TWO of them `<button onClick={navigate}>` rather than anchors — so the same idea behaved differently screen to screen and only some of them could be middle-clicked or opened in a new tab. One primitive now, `client/src/components/AdminEntityLink.tsx`, used by every surface; it takes children so a whole clickable ROW becomes an anchor instead of a button, and it refuses to render a link for an id it cannot address (a system action has no actor) because a dead control reads as a broken product. TWO SURFACES ALSO NEEDED SERVER WORK: the referral ledger could only print `#4127` for the REFERRED party, because the query joined `users` for the referrer alone — in a file whose own comment states the human-first rule for campaign names; it now LEFT joins the referred user under an alias, so a referral row cannot vanish because of a name lookup. FOUR SURFACES ARE DELIBERATELY NOT LINKED, each with a written reason in the test: AdminRegistrations (the name is a bulk-select checkbox label and the applicant opens through its own review surface), AdminPlacements (the placed entity links to its PUBLIC page on purpose — an administrator judging a placement wants what a customer sees), AdminRfqInvestigation (a Super-Admin-only reconstruction where ids are evidence, not workflow), and AdminUserDetail (it IS the record). AND THE PROBE FOUND A DEFECT NOBODY WAS LOOKING FOR: to give the audit trail a row to render it performed a REAL administrative action, and the row never arrived. `admin.verifyUser` changed `users.verified` and recorded NOTHING, while all nine of its neighbours over the same table record — so it was an oversight, not a policy. It is the worst one to have missed: the flag decides whether a provider is listed in the marketplace at all, and setting it qualifies a referral, which can grant a reward, so the platform could hand out a benefit with no record of the decision behind it. Both directions are now recorded (`account_verified` / `account_unverified` — removing verification takes a provider off the marketplace and is as consequential as granting it), and the rule is a census: no privileged mutation over `users` may change a row silently. Evidence: `server/adminEntityLinks.test.ts` (14), 5 mutations killed. `evidence/zg-adminidentity-run1.txt` / `-run2.txt` (14/14 twice, identical verdicts: four distinct real people seeded, each name read out of the rendered DOM as an anchor, one FOLLOWED to the account, the referred party rendered as “Nour Saad” rather than a number, every href addressing a real row, both the SUBJECT and the ACTOR of a real audit event reachable, and the same links under Arabic RTL). THREE OF THE PROBE'S OWN CHECKS WERE WRONG FIRST: it asserted identity links on an audit trail it had given no events to (raw SQL seeding writes none); it then read the DEFAULT tab, because `element.click()` does not open a Radix tab — that listens for pointerdown, so the probe reported the product as wrong while the identities were one real click away; and it looked for its event on page one of a paged, ordered trail that earlier probe runs had filled, instead of using the screen's own server-side search.
...
- [x] RFQ basket workflow: select/add/remove/quantity/notes/specifications/persistence/submit without duplicate RFQs. INVESTIGATED BEFORE ANYTHING WAS WRITTEN, and most of this line was already built - recorded rather than quietly ticked, because a line that reads as new work when it is not is how the same work gets done twice. `shared/rfqBasket.ts` holds the contract (MAX_BASKET_ITEMS 30, quantity bounds, `addToBasket` merging a repeated product+variant into ONE line with a larger quantity rather than two identical lines a supplier cannot read); `client/src/hooks/useRfqBasket.ts` exposes add / update / remove / specify / clear over localStorage, reconciles the basket's owner on identity change and clears it on sign-out; it is wired into Marketplace, ProductDetail and RFQPage. Notes and specifications both exist - the RFQ's own `description` on the form, and per-line `specifications` on each basket item. "Without duplicate RFQs" is enforced server-side: `rfq.create` looks for a RECENT IDENTICAL request before its transaction and returns that existing id instead of writing a second RFQ.
- [x] Complete RFQ Detail navigation and authorized requester/project/items/attachments/status/response action. Items, attachments (owner-only, behind the credit), status and the response action were already on the page; THE PROJECT WAS THE ONE ELEMENT GENUINELY MISSING, and it was missing in the worst way - `rfqs.projectId` was written by the form, indexed, and returned by `rfq.get` as a bare integer nothing rendered, so a buyer could link a request to a project and never be told which. Now resolved to a NAME and a link to `/projects/:id`. THE ACCESS CHECK IS THE DESIGN: linking happened at creation and membership can be withdrawn afterwards, so the title resolves only through `canAccessProject(..., 'read')` - the same canonical check every other project read uses - and the row is never touched when that fails, or the RFQ page becomes a way to keep reading the name of a project somebody was removed from. Two states are kept distinct instead of collapsed: a link the buyer can follow, or a plain "raised for a project you no longer have access to" - they created the link, so saying it exists leaks nothing, while rendering nothing would look like an RFQ that was never linked. A supplier sees no project line at all. Evidence: `server/rfqProjectContext.test.ts` (6), the access guard mutation-tested - dropping it turns two assertions red; `evidence/zg-rfqproject-run1.txt` / `-run2.txt` (14/14 twice against the running product with real accounts and real Chromium: the member sees the name and REACHES the project by clicking it; after removal the name is gone from the page entirely and the API returns `project: null, projectLinked: true`; a supplier sees no line and `rfq.get` answers NOT_FOUND). Three of the probe's own failures were the instrument and were fixed rather than reported as defects - a query POSTed instead of GET, and a readiness condition that read the DOM between the summary and the owner-only record - but ONE was a real defect in the new markup: the anchor was nested inside wouter's `<Link>`, which renders its own, producing invalid `<a>` inside `<a>` and no href.
- [ ] Supplier answer editing/moderation policy — retain as OWNER DECISION unless already resolved
  - Reconciled with evidence in `OWNER_DECISIONS.md`.
...
- [x] Admin analytics using real data only — the analytics themselves were already exemplary on commercial truth: MRR comes from `vendorSubscriptions` and never from the lossy event stream, every price resolves from shared/billing.ts so nothing is hardcoded, revenue counts a subscription only if `deriveBillingState` says it is paid RIGHT NOW, A TRIAL IS EXPLICITLY NOT REVENUE, and `arpv` and the median-days figures come back null rather than 0 so that “zero days to verification” and “nobody has been verified yet” stay different facts. Nothing fabricated, nothing to remove. THE DEFECT WAS THE OUTAGE PATH, and it was the worst instance of it in the product: `getCommercialKpis` answered MRR 0, ARR 0, zero paying vendors when the database was unreachable — the owner's revenue figure, on the screen they check revenue on, reading zero during a blip and indistinguishable from every customer having left. Same for `getChurn`. The `requireDb` rule that routers.ts has followed for forty-five call sites had never been applied to the modules BEHIND the routers. TWO MORE FOUND BY THE SAME SWEEP, one of them worse in kind: `countUsage` in billing/enquiries answered 0, which flows straight into `getEnquiryUsage` as used 0, remaining full, `limitReached: FALSE` — IT FAILED OPEN ON A PAID QUOTA, reporting a vendor who had spent their whole monthly allowance as having spent none of it, to them and to the check that decides whether another qualified enquiry may be opened. An honest failure costs a retry; a fabricated zero gives away paid leads. And every read in `vendorDirectory.ts` — the six the PUBLIC marketplace is built from — answered `[]`, so a visitor judging whether BuildHub has anyone on it saw a platform with no providers; the hub's headline COUNTS were fixed for exactly this one layer up, and the reads underneath them were not. Plus `placementPerformance`, the read living in a file of writers, which does not get their exemption. `getSubscription` and `getFounderOfferEndsAt` too: `null` there means “this vendor has no subscription”, which silently downgrades a paying vendor to free. THE RULE IS NOW A CLASSIFICATION, not a prohibition, because not every instance is a defect: each remaining `if (!db) return` in the server tree is SWALLOW (a fire-and-forget writer — failing a user's mutation because a metric could not be written is the worse outcome), CLOSED (a refusal: false, unavailable), or DEBT (a read that states a fact). A new file answering without a database cannot appear unclassified, a classification that stops applying must be removed, and a “writer” that returns DATA is caught — which is how the misclassification of placementAnalytics was found, since that file holds two recorders AND a read. Evidence: `truthfulEmptyStates.test.ts` now 39 assertions, `productAnalytics.test.ts` and `featuredPlacement.test.ts` restated — both PINNED the old behaviour (“returns a zeroed shape rather than throwing”, “returns nothing rather than throwing”) and both were written before requireDb existed; the half that was right is kept as its own test (a platform with NO subscriptions must still render zeros), and the outage half now asserts the opposite. 2 mutations killed.
- [x] ERROR-IS-NOT-EMPTY, server-side debt worked down 28 → 14, and the 14 that remain are CLASSIFIED rather than merely outstanding. Converted: the vendor FUNNEL (every stage reading 0 users — nobody ever registered, completed a profile or subscribed), event counts, milestone counts, a vendor's billing history, a vendor's declared CATEGORIES (answering none quietly removes them from matching), the reconciliation queue (empty means “every subscription is in order”, precisely the claim an unreachable database cannot support), and both master placement slots. THREE MORE WERE SECURITY- OR IDENTITY-SHAPED: `getUserByOpenId` returning undefined means “no such account” and is the BOTTOM of the chain that turned an outage into a sign-out — the layers above now classify a non-HttpError as “could not check”, and this is what raises one; `getUserByEmail` and `getUserByUsername` do the same on the sign-in path, where “no account with that email” is why somebody is told their credentials are wrong; and `revokeSession` SILENTLY NO-OPPED, so `auth.logout` cleared the cookie and reported success while THE TOKEN STAYED VALID everywhere else it had been copied — which is the entire reason server-side revocation exists. db.ts raises the same sentence through a local helper rather than importing the transport layer into the connection module, for the reason requireDb's own header gives. The remaining 14 are SWALLOW (fire-and-forget writers: the analytics recorder, the commercial audit helper, notification and history writers) or CLOSED (refusals: health false, storage proxy false, bootstrap “unavailable”), with one genuine debt left — `billing/service.ts` line 255, a subscription state write that returns null instead of refusing. Two of my own test assertions were FILE-SCOPED where they should have been FUNCTION-SCOPED and the classification caught both: analytics/events.ts holds a swallowing recorder AND reads that must refuse, and so does placementAnalytics.ts — a file can legitimately contain both, and the exemption belongs to the function that earns it, not to the file it lives in.
- [x] ERROR-IS-NOT-EMPTY: THE SERVER-SIDE CLASS IS CLOSED. The last classified debt was `applySubscriptionPatch` in billing/service.ts, and it was REMOVED rather than repaired. An unreachable database made it answer `null`, so a subscription change silently did not happen — neither fail-open nor fail-closed but FAIL-SILENT, which is the worst of the three on a commercial mutation. Repairing it would have preserved the wrong thing: NOTHING CALLED IT. The real write path is `billing/lifecycle.ts`, which does the same job inside a TRANSACTION with the row LOCKED (`.for('update')`), so two concurrent lifecycle actions on one vendor cannot interleave — this one took no lock. A second, weaker writer for the same table, reachable by import, is how the careful path gets bypassed later by somebody reaching for the obvious name. `billingAuthorization.test.ts` ASSERTED A PREMISE THAT WAS NEVER TRUE — “the only writes to vendorSubscriptions go through applySubscriptionPatch” — while lifecycle.ts has always written directly; its actual assertions were right and still pass, and the rule is now stated over the whole billing directory so a THIRD writer cannot appear beside them, with the lock and the domain-produced field set asserted on the real one. Also hardened `ensureSubscriptionRow`, as defence in depth rather than a behaviour change: `runLifecycle` already returns a stated rejection (“Billing storage is unavailable”) before calling it. ELEVEN instances remain in the tree and every one is SWALLOW (fire-and-forget writers) or CLOSED (honest refusals) — the DEBT class is empty and the census now asserts that it stays empty, so a new instance must be argued into a category rather than parked. Two of my own assertions in this pass were wrong in the way this project keeps finding: one matched the phrase “for update”, which the file's own header satisfies while EXPLAINING the lock in prose, rather than the `.for('update')` call.
...
- [x] Accessibility and mobile/RTL pass at 375/768/1440 for all modified areas
...
- [x] Promotion management accuracy/regression for Featured and Sponsored prime placement and lifecycle
- [x] Marketplace discovery reconciliation across Home, Products, Vendors, Designers, Finishing, and provider directories — MOST OF THIS WAS ALREADY RECONCILED and the survey says so rather than re-doing it: Home renders a figure only when it is a real, non-zero count and shows nothing otherwise; the designers and finishing directories render the SAME authorized vendor directory as /marketplace/vendors, filtered to a category providers actually declare, with reputation from verified reviews and verification from the compliance decision; the hub's product chips and search suggestions come from the one canonical taxonomy since CAT, carrying the canonical English name the marketplace filter and `products.category` both hold. ONE REAL DEFECT, in the four numbers at the top of the hub's section cards. Two of them counted real things — `productCategories.length` labelled "categories", `directory.length` labelled "vendors" — and TWO COUNTED A CONSTANT COMPILED INTO THE PAGE: `DESIGN_CATEGORIES.length` (14) labelled "disciplines" and `FINISHING_CATEGORIES.length` (21) labelled "services". With no designer on the platform the card still read "14 disciplines": a number that cannot move, in the same slot and the same typeface as one that can, so a reader comparing the four cards was shown depth the marketplace might not have. The hub was ALREADY computing the real `designers` and `finishing` lists from the authorized directory — it used them for the search suggestions and the featured strips and not for the headline figure. Both now count real approved, verified providers and are labelled as such in both languages. THE CHIPS STAY AND ARE NOT THE DEFECT: they describe what a section covers, they are Badges rather than controls, and the card as a whole is what navigates — `marketplaceData.ts` says so in its own header, and the test pins that no chip becomes a control that filters nothing. Evidence: `server/marketplaceDiscovery.test.ts` (11), 5 mutations killed, including a rule stated over the whole block rather than card by card so a NEW section cannot quietly reintroduce a compiled-in constant. `evidence/zg-discovery-run1.txt` / `-run2.txt` (14/14 twice, identical verdicts: the card reads 0 on an empty platform, TWO DESIGNERS SIGN UP AND IT READS 2 — which a constant cannot do — the finishing card that gained nobody does not move, the number agrees with the directory behind it, the Arabic label is genuinely Arabic and the number is the same fact in either language, and nothing overflows at 375/768/1440). Restoring the constant takes the probe to 12/14. A figure I wrote in three comments as "12" was wrong — the list has 14 entries — and the probe assertion that depended on it was corrected to check both real lengths rather than one guessed one.
- [x] Admin global search opens entity management for Users, Vendors, Products, Projects, RFQs, Quotations, Disputes — the search EXISTED and was sound where it reached: five segments, each gated by the permission that already governs that record type, an omitted segment reported as omitted rather than empty, and explicit column allowlists. FOUR REAL DEFECTS, all of them about the half it did not reach. (1) IT COULD NOT FIND A CASE. Disputes, support tickets and vendor enquiries had no segment at all, so an administrator holding DSP-2026-000012 on the phone — the reference the notification, the parties and the console all use — had nowhere to type it. Three segments added, permissions read off the procedures that already serve those records (admin.disputes and admin.supportTickets are adminWith('support.manage'), admin.enquiryList is marketplace.manage), never invented. The enquiry segment goes through the CANONICAL enquiryList rather than a second query: it already resolves state from the pair, already parses a pasted ENQ-501-10, already escapes through containsTerm, and a hand-rolled copy here would be a second definition of what an enquiry is. (2) THE RESULT WAS ADDRESSED BY ROW ID. A dispute is its reference to everyone who has ever discussed it; the console led with #14. Hits now carry `ref`, the reference leads, the id is secondary technical metadata — and searching a VENDOR'S BUSINESS NAME found nothing at all, because only users.name/email/username were matched. The people segment now LEFT JOINs vendorProfiles (left, not inner — an inner join silently removes every homeowner from the console's only search) and labels a provider by its company with the person named underneath. (3) A BID'S DESTINATION WAS RECOVERED BY REGEX OVER ITS DISPLAY TEXT — hit.detail.match(/#(\d+)\)?$/) — so re-wording one label would have unlinked every bid in the search with nothing failing. The server now names the record a hit resolves against as a KIND, and client/src/lib/adminSearchDestinations.ts is the single map from kind to route, held as data beside adminNavigation.ts for the same reason: so a test can ask the question. (4) /admin/disputes/12 AND /admin/support/12 PARSED THE RECORD AND DROPPED IT. Only the enquiries section consumed it, so every case link — from search, from a notification, from an administrator pasting a URL — landed on an unfiltered queue. Both queues now take `openRecord` and open that case, as an effect rather than a seeded initial state so arriving at a second case while already on the screen opens that one. ALSO: the search card lived inside the DISPUTES tab, reachable only by an administrator who first went looking for disputes, and the RFQ investigation beside it has since grown its own typeahead over the same procedure — two search boxes, one procedure. It moved to Operations, where the console keeps its cross-cutting instruments. AND the section allowlist was a literal array inside AdminDashboard that duplicated ADMIN_NAV; it is now derived (ADMIN_SECTIONS), which is what makes 'does this link reach anything' answerable at all — /admin/:section/:record is a CATCH-ALL, so route registration alone says yes to /admin/dispute-cases/7. Evidence: `server/platformSearch.test.ts` (22, restated for eight segments — the per-role matrix, the no-query property extended to the raw-SQL path, and the two magic counts replaced by the segment list) and `server/adminSearchDestinations.test.ts` (10, new). 7 mutations killed across the two. THE DESTINATION TEST'S FIRST VERSION SURVIVED THE MUTATION IT EXISTS TO CATCH: it matched hrefs against App.tsx route patterns, and /admin/dispute-cases/7 matches the catch-all — it registers, then renders the overview. It now requires an /admin destination served only by a catch-all to name a section the console resolves. `evidence/zg-adminsearch-run1.txt` / `-run2.txt` (24/24 twice, identical verdicts: a real dispute and a real ticket found by reference and OPENED as cases, a business found by company name with the person named underneath, a MARKETPLACE_ADMIN told the case segments are WITHHELD rather than shown them empty, the server refusing admin.disputes directly at HTTP 403, the search API returning the reference in no hit they can read — with a positive control proving the same call DOES return it to an administrator who may look — and all three new segment labels rendering in Arabic under dir=rtl). THREE OF THE PROBE'S OWN CHECKS WERE WRONG FIRST AND WERE FIXED, NOT WAIVED. One asserted the reference appeared nowhere in the API response, and failed — because the response ECHOES the caller's own query back; a substring match over a body cannot tell a leak from an echo, so it now reads the segments. Two asserted the case had opened by looking at document.body.innerText, which is ALSO true of the queue behind it because the list renders every reference in its own column — both survived the exact mutation they exist to catch and now require the case's own panel. And the Arabic block ran as the SUPPORT_ADMIN: CDP cookies belong to the BROWSER, not the tab, so signing in the restricted administrators had replaced the session, and it reported a missing Arabic label for a segment that was never rendered. A fourth mutation appeared to survive and had not been tested at all — tsx does not hot-reload server modules, so the old build was still serving; re-run against a restarted server it fails 3 checks.
- [x] Search result STATUS badges render the raw database enum ('open', 'in_progress', 'awaiting_user') rather than a translated label, in every one of the eight segments. Pre-existing and consistent, not introduced by the search completion pass, and deliberately NOT half-fixed there: the eight segments carry eight different status vocabularies, some of which already have canonical label helpers (disputeLabels) and some of which do not, so translating three of them inside the search card would create the ninth vocabulary rather than reuse the eight.
- [x] ERROR IS NOT EMPTY, AND AN OUTAGE IS NOT A SIGN-OUT — a client-side census of the rule `truthfulEmptyStates.test.ts` already held for the server. 30 of 48 data surfaces distinguished a failed request from an empty one; the rest rendered a confident sentence on no evidence. SEVEN FIXED, chosen by what each one CLAIMED: AdminProjectDetail said “Project not found.” — a statement about a record's EXISTENCE, made to an administrator mid-investigation; PlacementPerformance said “No placements booked yet” under a heading about reach, a commercial zero on no evidence; the PUBLIC marketplace hub counted its `= []` defaults and told visitors BuildHub has 0 vendors and 0 designers (the same cards were fixed once before for counting a compiled-in CONSTANT — an empty array was being counted exactly the same way); the supplier dashboard said “No products yet. Add your first product” to a supplier with a full catalogue, which is how an outage produces a duplicate listing; both admin typeaheads said “No matching vendors/products”; and /pricing routed a failure into `isLoading || !data` so the one page that answers “what does this cost” hung on a spinner forever. Home was checked and LEFT ALONE — it hides its figures on failure, which is already honest. ALSO CONSOLIDATED: AdminDashboard and AdminRegistrations each carried their own identical copy of the failure sentence, both under a comment claiming it was “worded once, not per tab” — true inside each file, false across the two. THE BIGGER FINDING CAME FROM THE PROBE, not from reading code: with the database stopped, /admin/projects/27 REDIRECTED TO /auth. `isSessionRevoked` fails closed by answering “true”, the authenticator repeats that as “Session has been signed out”, `createContext` caught EVERY error and set `user = null` under a comment about public procedures, `auth.me` answered null, and the client reads that one procedure to decide who is signed in — so a database outage presented to a working administrator as a sign-out, and sent them to a sign-in screen the same outage made unusable. Fixed at all four layers WITHOUT weakening anything: the revocation check still refuses (it throws instead of asserting a sign-out that nobody checked — an exception stops a caller more firmly than a boolean), the context classifies HttpError as a real auth failure and everything else as “cannot tell”, the guards raise INTERNAL_SERVER_ERROR instead of the UNAUTHORIZED that drives the redirect, and `useAuth` gained a third state that DashboardLayout renders. `isAuthenticated` stays false throughout and every protected procedure still refuses — only the explanation changed. Evidence: `truthfulEmptyStates.test.ts` (33, census merged in rather than a second file on the same rule, with a written-down DEBT LIST that can only shrink — the test fails both when a new surface joins it and when a fixed one stays on it), `marketplaceDiscovery.test.ts` restated and given the failed-request rule, `productionHardening.test.ts` A2 restated at the stronger property (it must never RESOLVE FALSE, whatever the mechanism). 9 mutations killed. `evidence/zg-errorstates-run1.txt` / `-run2.txt` (12/12 twice, identical verdicts, against a REAL stopped MariaDB — healthy rendering proven first so a failure state is a CHANGE rather than all the screen ever draws). Three probe mutations: two killed, and the third SURVIVES for a recorded reason — `if (!db)` runs only when getDb() returns null, which needs DATABASE_URL unset, so the live outage takes the thrown-query path instead; the null-handle branch is killed by the unit tests, 4 assertions. The pricing branch is NOT claimed by the probe: `billing.plans` is an in-memory publicProcedure that no database outage can fail.
- [x] The STORAGE PROXY makes the same conflation, one layer over: `server/_core/storageProxy.ts` catches every error from `authenticateRequest` and returns null, which becomes 401. During an outage a signed-in user's own files are refused as “unauthorized” rather than reported as unavailable (503, which the proxy already speaks). Lower severity than the sign-out defect — the consequence is a broken image, not a lost session — and the proxy's 401/403/503 ordering is pinned by tests, so it wants its own pass rather than a widening of the auth-honesty commit.
- [x] ERROR-IS-NOT-EMPTY, COMPLETED: THE DEBT LIST IS NOW EMPTY. 13 more surfaces fixed and 5 taken off as census defects rather than product ones. The five came off because the RULE got precise, not looser: AdminDataQuality, AdminRfqInvestigation and VendorReputation destructure `error` from the query and render it; RFQRespondPage holds the query object and reads `summary.error` off it; and Home was listed because it carries the COMMENT “nothing where there is no count yet” — prose describing the very honesty being asked for, so the census strips comments now. Three spellings of the same observation, each learned by getting a file wrong first, and each tied to a QUERY — a census that accepted a bare `error` would have passed AdminFeaturedProviders, which holds an `error` state for its MUTATIONS while its query's failure went unobserved, which is the exact file the rule exists to catch. THE 13 FIXED, by what each claimed: “User not found.” (a claim about a PERSON, in the admin console, the same defect as the project page); “Product not found” on a PUBLIC product page reached from a shared link; ProductFormPage told a supplier their own product does not exist WHILE THEY WERE EDITING IT, because `isFetched` turns true after a failed attempt too; “No conversations yet” to somebody with an inbox full of quotations; “No work added yet. Add your first project” to a provider with a portfolio; “No milestones have been added yet” and “No progress reports yet” on the two tabs a client opens to find out exactly that; “No requests yet.” while a vendor name change was pending, which is how a second one gets submitted; “No enquiries yet” as a statement about the whole marketplace; empty commercial ledgers for sponsorships, featured providers, placements and the name-change queue; and a third copy of the vendor typeahead saying “No matching vendors.” AND THE CENSUS ITSELF WAS STRENGTHENED after a mutation survived it: disabling AdminUserDetail's error arm while leaving `isError: detailFailed` in the destructure passed, because observing a failure is not the same as ACTING on it — the file already knew this (`usersFailed` was once destructured here and used nowhere) and the rule did not. Every alias must now appear outside the line that created it. Evidence: `truthfulEmptyStates.test.ts` (34), debt list 18 → 0, 3 further mutations killed. Probe re-run 12/12 twice with identical verdicts — after its TEARDOWN was fixed: it waited a fixed 3s for MariaDB to come back, which was enough on one run and not the next, so two runs disagreed about a line that has nothing to do with what is being tested. It polls for readiness now.
...
- [x] Role-specific Quick Actions accuracy, and the menu entry that named a feature BuildHub does not have — the PROJECT MANAGER was offered “Team” in two places, the sidebar and the shortcut row, and BOTH navigated to /messages. Team / organization structure is an OPEN OWNER DECISION with nothing built behind it, so the word promised a feature; and the one role with no Messages entry at all was the one whose Messages entry was wearing another name. RolePlatform's own comment states the rule, about a “Documents” shortcut fixed a pass ago: “a shortcut that lands somewhere other than its label is the same defect as one that lands nowhere” — and “Team” sat two lines above that note. Both now say Messages, which is what they do. The unbuilt destination stays unbuilt and tracked rather than prototyped in a label. THE RULE IS NOW A TEST, added to `roleWorkspaceNavigation.test.ts` rather than a second file about the same thing: every scroll destination in every per-role array must be a section THAT role renders. The existing checks measured ROLE_SECTIONS, the registry, while the shortcut arrays call goToSection directly — so a shortcut naming a section its role does not render was invisible to every test in the file. I ALSO REPORTED A SUPPLIER DEFECT THAT WAS NOT ONE: a grep window that landed on HomeownerWorkspace made it look as though only the homeowner rendered role-projects, so I removed a working “Projects” shortcut and replaced it — SupplierWorkspace renders that id too and the registry was right. Restored, and the mutation that SURVIVED is what exposed my error rather than the test's. Evidence: 43 assertions in that file, M3 kills the Team regression in both places; the shortcut-destination rule is checked for all six roles with a positive control that both per-role arrays (stat tiles AND shortcuts) are really being read — the first version of the slicer took only the first array it found, which is the tiles, so the shortcut it existed to check was never read.
...
- [ ] Historical owner decisions: project.spent vs expense log, renovation/finishing preset, mandatory product fields, provider comparison surface, RFQ budget exposure, messaging relationship policy, supplier answer edit/moderation policy
  - Reconciled with evidence in `OWNER_DECISIONS.md`.
- [x] ACCESSIBILITY — a RENDERED pass, not a scanner score: real pages loaded as a real administrator, the accessible name of every interactive element computed the way a screen reader does, the heading outline walked, focus tabbed through the page, and a dialog opened and followed. TWO REAL DEFECT CLASSES FOUND. (1) FOUR RADIX TOGGLES ANNOUNCED AS “button”. Switch and Checkbox render a <button>, and the WRAPPING <label> each of them sat in does not name one — label/for names form controls — so the visible text was there for sighted readers only. The Data Quality dummy-accounts switch, the platform Settings switches, and all three registration checkboxes including the per-applicant one, which now names the applicant. (2) EVERY DIALOG IN THE PRODUCT DROPPED FOCUS ON CLOSE. Almost all 32 are driven by STATE rather than a DialogTrigger: closing sets the state back to null, which re-renders the list the opener lives in, Radix's saved node is replaced, and focus falls to <body> — a keyboard user who opened a row's dialog was returned to the top of the page. Proved in a browser (Escape closed it, activeElement was BODY) and fixed ONCE in the primitive. THE FIX TOOK FOUR ATTEMPTS AND EACH FAILURE WAS AN ORDERING LESSON: a mount effect runs AFTER Radix moves focus, so it saved the dialog's own Close button; a focusin listener is right in a real browser and INVISIBLE to a headless one (document.hasFocus() is false, so focus events never fire, though activeElement still updates) so it could not be proved; a useRef initialiser runs at PAGE LOAD, because DialogContent is in the tree the whole time and only Radix's portal renders conditionally, so it saved <body>. A callback ref on the content node fires on insertion, before any effect, when activeElement is still the opener. Restoration is by identity while the node is attached and by data-testid when it is not, and only when focus was actually dropped on <body>. Evidence: `evidence/zg-a11y-run1.txt` / `-run2.txt` (15/15 twice, identical verdicts), `server/accessibilityContracts.test.ts` (6), 2 mutations killed in source and live. ONE PROBE DEFECT TOO: the dialog check first matched /audit/i and picked “Export Audit PDF”, which downloads a file and opens nothing — so the dialog checks failed against a control that was never going to open one, and the focus-restoration check then PASSED because focus had never left. NOT COVERED, deliberately: contrast ratios need a paint-level measurement this harness does not have, and inferring them from CSS text would be a score rather than a finding.
- [ ] Accessibility/mobile/RTL evidence separated into responsive implementation vs VISUAL QA
- [x] `/enquiries` SHOWED THE SAME SIX REQUESTS TWICE, and the fix is a partition rather than a filter. The page stacked `QualifiedEnquiries` (reading `rfq.eligible`) above `EnquiryQueue` (reading `rfq.queue`), and `rfq.eligible` was `listEnquiryQueue` filtered to `rfqStatus: 'open'` — which is NOT the question its own name asks. An open request the provider had already opened, quoted and WON came back in a list titled "the open requests this provider can act on now", so both cards rendered the same rows in two different vocabularies. Nothing was missing; everything was doubled, which is worse, because the reader has to work out whether the two lists disagree before they can trust either (§71). NOW: one query, one component, two scopes — **Opportunity Centre** (what can still be taken) and **My Leads** (what has been taken, and how it ended). The split is `ENQUIRY_OPPORTUNITY_STATES` / `ENQUIRY_LEAD_STATES` in `shared/enquiryStates.ts`, applied BY THE SERVER via `scope`, so a client cannot construct two views that overlap — which is how the duplicate arose the first time. AN EIGHTH STATE WAS NEEDED AND IT IS A REAL ONE: an untouched invitation read as `opened`, telling a supplier a lead was in their record that they had never seen, and putting an untaken offer on the same list as work they had paid for. §23 names Invited as a canonical opportunity state; `invited` is now it, ordered after `opened` in the CASE so that OPENING an invitation moves it across (`markInvitationViewed` advances `rfqSuppliers.status` off `'invited'`). ALSO FOUND WHILE THERE: `enquiryQueueSummary` seeded its counts from a hand-written `{available, opened, quoted, declined}` — four of a vocabulary that had grown to seven — so every state the literal forgot came back `undefined` from a function typed as returning numbers, and a chip reading `summary[state] ?? 0` printed a confident 0 over a pipeline that had never been counted. Seeded from `ENQUIRY_RESPONSE_STATES` now. AND THE WORKSPACE RENDERED A THIRD COPY: `RolePlatform` mounted the full inbox — allowance meter, rows, open action — while `/enquiries` rendered it again; §33 says a dashboard summarizes and the dedicated page manages, so it is now `EnquirySummaryCard` (counts from the same `rfq.queue` summary, a failed read rendering as a failed read rather than four zeroes) with a View All. THE DEEP LINK IS BETTER THAN IT WAS: `?rfq=` still selects and highlights, and because the page knows which half the request landed in it SAYS so, instead of leaving the provider to scan two lists. Evidence: `server/enquiryStates.test.ts` (11, new — the partition asserted as a partition, with both failure modes mutation-killed: a state in NEITHER half hides a request completely, a state in BOTH puts it back on the screen twice), `server/enquiryQueue.test.ts` and `server/providerResponseHandoff.test.ts` restated at the new structure rather than deleted, `evidence/zg-enquirysplit-run1.txt` (29/29 against real rows and a real browser). THE PROBE'S FIRST RUN FAILED FOR TWO REASONS WORTH RECORDING. It built its test request on a declared category — and `vendorCategories` is EMPTY in this dataset, so the request arrived nowhere and every assertion under it would have passed for being unreachable rather than for being right; it reaches the supplier by INVITATION now, which needs no declared category and exercises the new state. And its "never says EGP" checks accepted `MISSING` as a pass, which is a check passing because the element it was reading does not exist; they require the element now. Its disjointness claim is also guarded for non-vacuity — disjointness over two empty lists is a pass that means nothing, and it is exactly the pass a broken scope filter would produce.
- [x] A SAUDI BUDGET WAS LABELLED IN EGYPTIAN POUNDS, on the screen where a supplier decides what to bid. The enquiry detail rendered `{t('common.egp')} {Number(detail.budget).toLocaleString()}` — the digits from the record, the currency from a TRANSLATION KEY, which cannot know what the record is denominated in. At today's rates a 500,000 SAR request read as roughly a fifth of its real size. CLAUDE.md §87 settles where the answer comes from: the RFQ decides the currency and every quotation inherits it, never the supplier's country and never BuildHub's subscription billing currency. The whole path is now pinned — `rfqs.currency` selected in `listEnquiryQueue`, returned by `rfq.eligible` and `rfq.queue`, formatted through the canonical `formatMoney`. THE COUNTERPARTY'S SCREEN HAD THE SAME DEFECT AND IT WAS WORSE: `QuotationComparison` already formatted every BID through `formatMoney(q.price, q.currency)` while the BUDGET they are all compared against, and the under/over-budget difference, and the accept-confirmation price, came from the EGP key — so a Saudi request showed SAR bids beneath an EGP budget line and an EGP acceptance dialog. `rfqCurrency` is threaded from both call sites. AND THE REMAINING COUPLING IS NOW WRITTEN DOWN RATHER THAN INVISIBLE: the census in `marketReadiness.test.ts` hunts the literal `'EGP'` and cannot see a lowercase translation key, so seventeen call sites were invisible to every guard in the file. `COMMON_EGP_CALLERS` is a closed list of five files that can only shrink — a new one fails the suite, and a stale entry for an already-fixed file fails it too (both mutation-killed). WHAT SURVIVES IS PROJECT MONEY, AND IT IS NOT A RENDERING FIX: a project carries a currency (0058), but the homeowner and workspace dashboards SUM across projects, and a total over projects in two currencies is not a number — adding EGP to SAR produces a figure that is wrong in every currency. The honest fixes are grouping by currency or showing per project, which is a layout decision on the homeowner's main screen, so it is recorded rather than guessed at. Evidence: `server/enquiryMoney.test.ts` (12, new), `server/marketReadiness.test.ts` (50), `evidence/zg-enquirysplit-run1.txt` checks 20–24 — the budget rendered as `SAR 500,000` in English AND under Arabic/RTL, which is where a fallback to `جنيه` would have shown up.
- [x] SAVED → RFQ NOW COMPLETES, and the entry above it recorded why it did not. The shortlist worked and its CTA navigated to `/rfq` — a LINK, not a journey: the buyer arrived at an empty create form and the suppliers and products they had spent two days comparing were left behind on the page they came from. Every piece worked; nothing connected them, and it looked finished from either end, which is exactly the failure §6 exists to catch. NOW: tick what you want (defaulting to everything), press ONE button, and the selected products become basket lines through the canonical `rfqBasket` reducer while the selected providers become real invitations through `rfq.inviteSupplier`. No second path from this page (§11) — nothing here decides who may be invited; the server authorizes each invitation one at a time exactly as it does from a storefront. THREE THINGS THE OBVIOUS IMPLEMENTATION WOULD HAVE GOT WRONG. (1) `?invite=` took ONE id, which is the wrong number for the journey — a buyer shortlists eleven suppliers precisely so they can ask several. It takes a BOUNDED list now (`MAX_CARRIED_INVITATIONS = 10`), parsed once in shared/ because a URL is user-controlled input and a shortlist holds 200; the old single-id spelling still works, so every `/rfq?invite=5` link in circulation keeps working. (2) SENDING THE INVITATIONS AS ONE BATCH would lose three good invitations to one provider who is not approved to receive requests — `Promise.allSettled`, each outcome reported, the count announced is the count that SUCCEEDED and each refusal is named with the server's own reason. (3) SELECTION IS HELD AS EXCLUSIONS, not as a selection set, so an item saved while the page is open is included — the buyer who ticked nothing meant their shortlist. The form also NAMES the suppliers it is about to invite before submission (a form silently carrying four invitations is the same defect as a basket that silently contains things), from the buyer's OWN shortlist rather than a new public read, and an id that is not on it shows its reference rather than a name this page cannot prove (§68). It says the request stays public to every matching provider, because a buyer who thinks an invitation NARROWS the audience has been told something false. Evidence: `server/savedToRfq.test.ts` (20, new — the parser hardened against every non-id spelling, and the writer bounded by the same number as the reader so ids cannot be dropped silently between them), `evidence/zg-savedtorfq-run1.txt` / `-run2.txt` (28/28 twice, identical verdicts): a real buyer ticks one of two shortlisted products, presses one button, and the request is posted with exactly ONE rfqItems row, a real rfqSuppliers row, and THE SUPPLIER SEES IT IN THEIR OPPORTUNITY CENTRE as an allowance-exempt invitation. TWO PROBE DEFECTS, both of which would have produced a green run that proved nothing. It found the submit button by matching any enabled button whose text contained "post" — and clicked the trigger that OPENS the dialog, so it reported a successful submission of a request that was never sent; only the missing database row caught it. And its central control — "the unticked product did not come along" — searched the basket's rendered text for the product's ID, and the basket has never rendered an id, so the string was never there and the check passed whatever the page did. It SURVIVED the exact mutation it exists to catch: with `isPicked` forced to `true` the page carried both products and that check still passed. It compares NAMES and counts lines now, and the same mutation is killed by 5 checks.
- [x] MIGRATION STATUS VOCABULARY CORRECTED. 0056–0059 were described as applied "LOCALLY ONLY" in `OWNER_DECISIONS.md` and in the commits, after they had already been pushed — which understates where they are and invites somebody to apply them a second time. §2 forbids collapsing IMPLEMENTED / COMMITTED / PUSHED / DEPLOYED / STAGING VERIFIED, and that applies to migrations exactly as it applies to features. All four are now reported at their furthest PROVEN state: **PUSHED — applied locally, not yet staging-verified**. PUSHED means the file is in `origin/claude/buildhub-global-release-candidate` and the schema change has been applied to the local database and exercised by the suite; it does not mean any deployed environment has run it. NOT-YET-STAGING-VERIFIED is the honest remainder, because Render is `connect_rejected` to this environment by organization egress policy and no deployed SHA can be observed from here. Nothing becomes STAGING VERIFIED until a deployed `/version` reports the exact SHA that carries them.
- [x] A TERMINAL REQUEST WAS STILL BEING SHOWN AS AN ACTIONABLE LEAD. Every arm of the response-state expression that consulted `rfqs.status` ALSO required a quotation — `when quotations.id is not null and rfqs.status = 'awarded' then 'lost'`, and the same shape for `closed` — so a supplier who OPENED a lead, spending a credit on it, and never bid fell straight through all of them to `opened` and stayed there for good. The customer withdrew the request three weeks ago, or awarded it to somebody else, and the supplier's pipeline still showed work waiting for them. WORSE FOR AN INVITATION NEVER TOUCHED: it sat in the Opportunity Centre being offered as takeable, behind an "Open enquiry" button that could only ever be refused — the dead control §13 exists to stop. THE FIX IS ARM ORDER, two quotation-free arms placed after every quotation arm and before `opened`/`invited`: a WITHDRAWN request reports `closed` ("Request withdrawn"), which is true whether or not this supplier quoted because the fact being reported is what the CUSTOMER did; an AWARDED request this supplier has no quotation on reports the new `unquoted` ("Did not quote"). IT IS DELIBERATELY NOT `lost`. "Not selected" is a statement about a competition, and telling a supplier they were beaten in one they never entered is a fabricated outcome (§68) — and it is exactly the kind a supplier acts on, by writing off a customer who never rejected them. `declined` still outranks both, because it is the supplier's own recorded decision and a later award does not rewrite it. THE PARTITION MOVED WITH IT: `closed` and `unquoted` are LEAD states even when the supplier did nothing at all, because the question the Opportunity/Leads split answers is "can this still be taken", not "did you touch it" — so a concluded request leaves the Opportunity Centre and appears in My Leads instead of vanishing. Evidence: `server/enquiryLifecycle.test.ts` (14, new — it asserts ARM ORDER rather than mere existence, because order is the whole mechanism; moving the terminal arms below `opened` makes them unreachable while every state still exists, and that mutation is killed). `evidence/zg-leadlifecycle-run1.txt` / `-run2.txt` (31/31 twice, identical verdicts, against real rows through the real query): both named transitions proven, the invitation proven to LEAVE the Opportunity Centre and arrive in My Leads, a declined invitation proven to survive a later award, and Won / Not selected / Request withdrawn / Did not quote proven to remain four distinct outcomes so the fix did not flatten the end of the funnel. Restoring the original defect takes the probe to 21/31 and it reports the owner's exact symptom (`[opened]`, `[invited]`); collapsing `unquoted` into `lost` takes it to 27/31. THE CONTROL IS ASSERTED TWICE, first and last: a state expression that had simply started answering "closed" for everything would pass most of the terminal checks, so live requests are re-checked at the end to prove terminal means terminal rather than always. TWO PROBE DEFECTS FIXED FIRST. Its Won/Not-selected scenario inserted a quotation with no `qualifiedEnquiries` and no `rfqSuppliers` row — a state that cannot arise, since a supplier cannot quote without first opening or being invited — so the reachability filter correctly excluded it and three checks read ABSENT. Two failed honestly; the third, "is never collapsed into Did not quote", PASSED, because 'ABSENT' is indeed not 'unquoted'. A check a missing row can satisfy is not a check, so ABSENT is now refused outright.
- [x] SUPPLIER SHOWCASE (North Star 15) — the supplier's own emphasis on their own storefront, and the FENCE around it is the feature. §18 keeps three things apart: FEATURED is BuildHub editorial curation, SPONSORED is a commercial grant bounded by dates and revocable, and a SHOWCASE is the supplier saying "start here" on the page that is already theirs. THE DANGER IS PRECISE, and it is why most of the assertions are negative: if a supplier's own pick could influence marketplace discovery, category ranking or search order, they would have granted themselves a placement — Sponsored inventory with no admin decision, no period, no revocation and no label — and BuildHub could no longer answer "why is this supplier above that one", because the ordering rule §18 states (FEATURED → labelled SPONSORED → ORGANIC) would have a fourth, invisible competitor in it. SO IT IS CONFINED BY CONSTRUCTION: the only reader is keyed by one supplier's id and serves one page, and the strongest test is not "the marketplace filters it out" but "the marketplace has never heard of it" — a module census pins the exact four files that may import it, so a fifth importer fails the suite. A SEPARATE TABLE, NOT A `vendorSponsorships` ROW: that table carries grantedBy, grantedReason, revokedAt, revokedBy, startsAt, endsAt, priority, package and surface, every one an ADMIN decision about a SHARED surface, and a showcase has none of them; §11 says reuse a canonical system, not put unlike things in one table because the columns nearly fit — the cost here would be a self-selected row sitting in the store the placement engine reads. Ownership is re-derived from the target table on every write against the SESSION's id (the three tables disagree about the owner column — products.supplierId, serviceOfferings.providerId, portfolioItems.userId — which is exactly what a single hand-written check gets wrong once), publication is checked in the SAME WHERE so the two cannot drift, and the READER re-checks both so a product archived after being showcased leaves the storefront with nothing having to run. Bounded at six, de-duplicated (a duplicate silently costs a third of the showcase), wholesale transactional replacement so positions are contiguous by construction. Discoverable in Settings beside the service catalogue and portfolio, gated to accounts that HAVE a storefront; the buyer sees "Selected by this supplier · Not a BuildHub recommendation, and not a paid placement", and the supplier is told it does NOT change their marketplace position. Evidence: `server/supplierShowcase.test.ts` (29), `evidence/zg-showcase-run1.txt` / `-run2.txt` (26/26 twice, identical verdicts) — the IDOR attempted for real (supplier A cannot showcase supplier B's product; nothing stored, verified in SQL rather than from the response, with the POSITIVE CONTROL that the identical call with their own product succeeds), an unpublished product refused, the cap held server-side, the provider directory AND marketplace product order captured before and after a save and proven unchanged, a withdrawn product proven to leave the public page while the OWNER is told and a VISITOR is not. THREE PLATFORM GUARDS FIRED AND WERE ANSWERED RATHER THAN SUPPRESSED: the public-surface roster, the id-taking-procedure isolation matrix and the migration journal each required a written reason for the new public read. TWO OF MY OWN GUARDS WERE VACUOUS FIRST. The ownership assertion was written over the whole module and SURVIVED the mutation it exists to catch — deleting `eq(products.supplierId, userId)` from the write gate left all 29 tests green, because the identical string still appeared in `listShowcaseCandidates` further down the file; sliced to `ownedVisibleIds` and given a count of all three owner checks, it now kills that mutation and the service-ownership one. And the probe's ranking comparison read `.rows ?? .vendors ?? []` from a procedure that returns a BARE ARRAY, so it compared two empty lists and "unchanged" was true of nothing — its own non-vacuity guard caught it, and it now asserts the directory has at least two rows before the comparison is allowed to count.
- [x] SUPPLIER MARKETING CENTER (North Star 16, §89) — a COMPOSITION, not a new domain. Fresh audit first: the placement grant engine (`vendorSponsorship.ts`, `placementBooking.ts`), the label vocabulary (`shared/placement.ts`), the analytics contract (`shared/placementAnalytics.ts`), the `analyticsEvents` store and the Admin report all already existed; what did not exist was any supplier-facing destination answering "how is my business being promoted". So this adds NO table, NO migration, NO event type and NO metric — `placementPerformance()` gained a `scope` ARGUMENT rather than a fork, because two readers of placement performance can disagree about what a CTR is, and a supplier and an administrator reading different numbers for the same placement is how a commercial dispute becomes unresolvable. It answers the five questions §89 names: what is promoting me, what is active, what is its scope and expiry, how is it performing from REAL recorded events, and what can I legitimately do next. WHAT IS DELIBERATELY ABSENT: no budget, CPC, CPM, spend, revenue, GMV, ROI, projected reach or trend — BuildHub has no payment provider, so each would be invented, and a supplier acting on a fabricated ROI would be making real decisions on something BuildHub made up (§10, §30). The empty state offers no "buy placement" button for the same reason; it names the legitimate routes (complete your profile, publish your catalogue, choose your highlights). THE THREE KINDS STAY APART (§18): Featured and Sponsored are counted separately and never summed — an editorial pick's impressions are not advertising inventory — and each is described by WHO decided it; the Supplier Showcase is reported beside them and explicitly labelled as the supplier's own choice that does NOT change marketplace ranking, so it cannot be read as promotion granted. A REAL FINDING WHILE MEASURING: `rate(0, 1)` is 0%, which is true arithmetic but statistically worthless — "0.0% click rate" over one impression reads as "nobody clicks this", a claim the evidence cannot support (§68). Added `MIN_RATE_SAMPLE`/`rateIsMeaningful` to the SHARED module and gated the DISPLAY, leaving the counts and the computed rate untouched so the supplier's figures and the administrator's stay the same numbers. Evidence: `server/vendorMarketing.test.ts` (25), `evidence/zg-marketing-run1.txt` / `-run2.txt` (36/36 twice, identical verdicts) — a real placement created per supplier, scope and expiry rendered, ONE recorded event proven to move the number by exactly one, the supplier's impression count and click rate proven IDENTICAL to the Admin report's, the cross-supplier leak attempted both ways with positive controls, and a statistically worthless 100% proven to render as "Not enough data". THREE PROBE ASSUMPTIONS WERE WRONG AND THE PRODUCT WAS RIGHT: it assumed supplier A had no Featured placement (A holds a real `source='ADMIN_GRANT'` row, which the canonical `placementLabel` correctly calls FEATURED because only PAID_SPONSORSHIP buys the word "Sponsored"), it demanded `ctr === null` where a true zero exists, and it read one supplier's rendered text after navigating to another's page. AND A MUTATION SURVIVED, which produced the most useful assertion in the file: replacing `.where(scopeClause(scope))` with `.where(undefined)` — so the platform's entire placement performance is computed on every supplier page view — left the probe at 36/36, because the supplier's placement LIST comes from the scoped outer query and `placementPerformance` only supplies metrics looked up BY ID. Nothing leaked, so nothing failed. Scoping there is defence-in-depth and a performance guarantee, and both were invisible to every behavioural test; defence-in-depth that nothing checks is just code, so it is now pinned directly, and removing the OUTER scope is separately proven to leak (probe 33/36, reporting `LEAKED #65`).
- [x] SPONSORED / FEATURED PRESENTATION AND INTEGRITY (North Star 17, §89) — audit-first, and the audit said the engine was already right, so this is a VERIFICATION pass rather than a rebuild. `applyBoost` already refuses to inject (only ids the organic query returned can be lifted), caps promotion at a third of the page and at the surface's own capacity, keeps the page length constant so a lift is a MOVE rather than an insert, and returns the organic order untouched when there are no candidates; `placementLabel` already derives the public word from WHO PAID, with only `PAID_SPONSORSHIP` buying "Sponsored" and every unpaid source — editorial, referral reward, comped, admin grant — defaulting to "Featured" so an advertiser cannot silently inherit BuildHub's editorial credibility; and the Master slot HEADING already follows the same label as the badge, after an earlier pass fixed a paid booking rendering under the word "Featured" while the badge below it said Sponsored. What was missing was PROOF on the live surfaces and a pinned contract for the pair. Evidence: `server/placementPresentation.test.ts` (12, new — the two labels proven to be different WORDS and different ICONS in both languages, the Arabic proven to be genuinely Arabic, the icon proven `aria-hidden` because the word already carries the meaning, and the no-injection/cap/fail-safe rules pinned) and `evidence/zg-placementintegrity-run1.txt` / `-run2.txt` (18/18 twice, identical verdicts): a real paid placement created, the promoted provider proven to appear EXACTLY ONCE with the page length unchanged, the promotion proven to have actually moved them (position 2 → 0, so the single-appearance check is not vacuous), promotion proven not to exceed a third of the page, and the badge proven to carry its own word and icon rather than hue alone. THE INJECTION TEST IS THE IMPORTANT ONE AND IT SKIPPED TWICE BEFORE IT RAN: every provider in the dataset is approved and in the directory, so "a provider outside the directory" did not exist and the check reported SKIP. It now UNAPPROVES one for the duration — the realistic case, since an unapproved provider buying a placement is exactly what must not surface — with a CONTROL proving the directory really excludes them first, the paid placement then proven not to put them back, and a final check proving they return once approved, so the exclusion is the gate rather than a break. Restored in `finally`. TWO FURTHER PROBE DEFECTS, both of which had produced confident green: the Arabic-script assertion was written as a regex literal through a heredoc, which delivered a DOUBLE backslash into the file, so the character class held a literal backslash plus the letters u/0-6/F and it MATCHED THE ENGLISH WORD "Sponsored" — it now tests code points directly. And the language switch used the wrong localStorage key (`lang` rather than `buildhub_lang`), so every RTL finding was reported against a page that had never left English; it now waits for `dir="rtl"` and fails if the switch did not happen. With both fixed the Arabic label reads `إعلان مموّل` under a genuinely RTL document. Mutations: giving both labels the same icon, and letting an unpaid source say "Sponsored", are each killed.
- [x] REAL ANALYTICS FOUNDATION (North Star 18, §89) — audit-first again, and again the foundation was already sound: `analyticsEvents` is a real typed store with a closed vocabulary and a forbidden-metadata-key list, `placementPerformance` counts rows that exist, `rate()` returns NULL rather than a decorative zero, `analytics.myStats` is one session-scoped aggregate with a single definition of "accepted", and item 16 added `MIN_RATE_SAMPLE` so a true-but-worthless rate is gated on display while the counts stay identical between the supplier's screen and the Admin's. WHAT WAS MISSING WAS A RULE THAT STOPS THE NEXT SCREEN INVENTING ONE, because the defect never arrives labelled "fake" — it arrives as a plausible constant in a card that looks like every other card. So `server/analyticsHonesty.test.ts` (13, new) is a CENSUS over every product file rather than a spot check: nothing random may produce a number a user reads (one declared exception, the shadcn sidebar SKELETON WIDTH, and the exception is earned — the assertion checks it really is a width and not a figure); no ROI, GMV, CPC, CPM or ARPU may be computed anywhere, since none can be derived from this schema; nothing may claim a trend over a time series the product does not store; a rate must be NULL without a denominator and a REAL ZERO with one (both directions matter — hiding a true zero means a supplier cannot tell "no clicks" from "not measured"); analytics READERS must use `requireDb` so an outage cannot render as "0 impressions, 0 enquiries", which would be a commercial claim made on no evidence, while event WRITERS keep the opposite rule and answer `{recorded:false}` rather than failing somebody's page view. ONE FILE WAS FLAGGED AND THE CENSUS RULE WAS WRONG, NOT THE CODE: `server/analytics/kpis.ts` computes MRR — and it computes it from `vendorSubscriptions`, the contractual record, priced from `shared/billing.ts`, deliberately NOT from the event stream because an event stream may drop a write and an owner reporting revenue cannot work from a lossy log; it also counts trialing vendors separately, since counting a trial as revenue is the commonest way a subscription business lies to itself. MRR is therefore an EARNED exception rather than a declared one: a second test asserts it is still backed by `vendorSubscriptions` and still not by `analyticsEvents`, so the exception cannot survive the file changing. Mutations: a fabricated ROI on a new surface, a randomised metric, and revenue re-derived from the event stream are each killed.
- [x] HOMEPAGE: THE CONNECTED SOURCING JOURNEY (North Star 19, §89) — the page carried a six-card "Features" wall AND a four-card "Four Steps to Success" wall. Between them they listed capabilities twice and connected none of them: a reader learned that BuildHub HAS an RFQ system, not that posting one puts their requirement in front of matching suppliers and brings back comparable prices. Two walls of equal cards is exactly the shape §73 names and §89 forbids. BOTH ARE REPLACED BY ONE SECTION that carries a single idea: EVERY STAGE STATES BOTH SIDES — what the buyer does, and what the supplier then sees. That is §6's "buyer action must become supplier reality", and it is the thing a feature list structurally cannot say. The six stages are the ones §89 names (discover → shortlist → RFQ → compare → collaborate → build), rendered as a numbered CHAIN rather than a grid because the order is the argument, and as an ordered list so the sequence exists for a screen reader too. NO FABRICATED PROOF (§75): no customer logos, testimonials, "trusted by" or invented counts — every stage LINKS to the real destination, so the claim is checkable by following it rather than by believing a number. The dead constants for both old walls were removed rather than left behind. A REAL DEFECT THE PROBE CAUGHT, AND ONLY A BROWSER COULD: the final stage pointed at `/projects`, which has NO route — only `/projects/:id` is registered — so the last promise on the homepage landed on "Page Not Found". The SPA answers HTTP 200 for every path, so curl reported all five destinations healthy; the probe WALKS each link in a browser and reads the rendered page, which is what found it. Corrected to `/dashboard`, the destination the signed-in navigation already uses for Projects. Evidence: `evidence/zg-homejourney-run1.txt` / `-run2.txt` (15/15 twice, identical verdicts) — all six stages in order, all six proven to state the supplier's side explicitly, all five unique destinations proven to render a real page, both old walls proven gone, no fabricated social proof, genuine Arabic under a genuinely RTL document, and no sideways scroll at 375px with every stage still reachable. Mutations: pointing a stage at a dead route, and dropping the supplier counterpart line, are each killed.
- [x] SYSTEMATIC VISUAL / PRODUCT-QUALITY GATE (North Star 20, §89) — a sweep over 10 routes × 2 widths × 2 languages (180 checks), covering public, buyer and supplier surfaces, checking the things §48–§79 make release criteria rather than polish: the page actually RENDERED and reached the path it asked for (every other finding is worthless otherwise), exactly one H1, no sideways scroll at 375px, no raw enum or snake_case in visible text, every image declaring `alt`, every icon-only control carrying an accessible name, and genuinely RTL Arabic. FOUR REAL ACCESSIBILITY DEFECTS FOUND, and all four were invisible to every existing test because NONE of them is visible text. (1) The shadcn sidebar primitive hardcoded "Toggle Sidebar" in three places — an `sr-only` span, an `aria-label` and a `title` — so on every Arabic page the one control that opens the navigation announced itself in English; it is the ONLY name a screen-reader user gets. (2) `DashboardLayout`'s own mobile toggle did the same with "Toggle navigation". (3) The marketplace product cards had `aria-label="wishlist"` and `aria-label="compare"` — untranslated AND non-specific, so twelve identical buttons announced themselves identically, which is the defect §62 exists to stop. (4) SONNER NAMES ITS OWN REGION: the toast library injects `<section aria-label="Notifications alt+T">`, so a screen-reader user on an Arabic page heard the toast region announced in English on every screen in the product — including the keyboard hint, the one part they actually needed translated to use it. THE GATE ITSELF TOOK THREE ATTEMPTS TO BECOME MEANINGFUL, which is the more useful lesson. A whole-page Latin-text scan reported 20 "untranslated" findings and nineteen were somebody's NAME — a buyer called Dalia, a business called Zenith Marble Works, an email, a city. Scoping to UI CHROME (accessible names and navigation, which BuildHub writes and a user never does) cut it to 8, and all 8 were data legitimately carried INTO a label. The final rule separates template from data: a Latin word that also appears in the page's visible text is data; a Latin word appearing ONLY in an accessible name is untranslated copy. That distinction is exactly what the four real defects have in common — "Notifications alt+T", "Toggle navigation", "wishlist" and "compare" appear in no visible text anywhere, because they are chrome. AND IT CAUGHT A DEFECT I INTRODUCED WHILE FIXING ONE: my new product aria-labels used `product.name` while the Arabic card renders `nameAr`, so a screen-reader user was told a different product's name than the one displayed — the same mismatch the image `alt` two elements above had already avoided. TWO PROBE BUGS FIRST: `localStorage` on a fresh tab's `about:blank` origin throws a SecurityError, so the whole sweep died on line one with an uninformative "Uncaught" (fixed with a warm-up navigation); and every check asserts the route actually arrived before its findings count, because a redirect to `/auth` would pass "no overflow" and "no raw enum" trivially. Evidence: `evidence/zg-visualqa-run1.txt` / `-run2.txt` (180/180 twice, identical verdicts). Reverting the toast-region fix alone takes the sweep to 160/180.
- [x] FINAL ACCEPTANCE GAPS BEFORE THE VISUAL RELEASE GATE (owner correction). FOUR ITEMS, and the census in each one found a FAMILY rather than the instance the owner named. (1) HOMEPAGE TRUTHFULNESS: the CTA read "Join thousands of users who trust BuildHub to manage their construction projects" — two unevidenced claims in one line, how many people use BuildHub and how they feel about it — and the sign-in panel carried the same sentence. The census then found four more "thousands" claims and, unexpectedly, SEVEN FABRICATED TRUST BADGES sitting unused in the dictionary: `designersDir.badgeAwardWinning`, `badgeTrending`, `badgeRecommended`, `vendorsDir.badgeTopRated`, `badgeRecommended`, `finishingDir.badgeTopRated`, `badgeFastResponse`, plus `marketHub.awardWinningTitle`. None was rendered by any component — a loaded gun rather than a live lie, and "Top Rated" is a ranking BuildHub cannot defend while "Fast Response" is exactly the response metric §68 allows only "where statistically valid". All removed; `badgeNew` deliberately KEPT, because it is derivable from a creation date. Two orphan `// ── Testimonials ──` section headers removed as well, left behind when the testimonial keys went. The replacements are capability-based and checkable by using the product rather than believing a number. `server/publicCopyTruthfulness.test.ts` (14, new) is the regression, and it learned one lesson while being written: matching on the badge VALUE text flagged four legitimate SORT labels, because "الأعلى تقييماً" is both the badge "Top Rated" and the sort "Highest rated" — sorting by real review data is honest, so the rule matches the KEY NAME, which is unambiguous. (2) VISUAL-QA SCOPE: the sweep had no Admin route and tested only healthy rendered states. Now 22 routes × 2 widths × 2 languages plus the six state classes §89 names — 434 checks. Extending it to the Admin control plane found FOUR REAL DEFECTS the public sweep never could: every Admin destination rendered with NO H1 AT ALL (nine routes, plus User 360, so the console the owner operates daily had no page landmark and its outline began at level 2); the audit trail printed `row.action` RAW in four places including its filter dropdown's option labels, so an operator read `admin_password_reset_requested` in a badge; the Insights page printed `row.eventType` raw as `user.signed_in`; and the Admin attention badges' only explanation of what a number counts was an English server string rendered as a `title`, unreadable on an Arabic screen. Fixed with two new exhaustive bilingual vocabularies — 77 audit actions and 26 analytics events, both pinned by tests that fail if an event ships without a label, because a new action is a one-line addition that would otherwise silently put snake_case back on an operator's screen. (3) AND THE STATE-CLASS PASS FOUND THE MOST SERIOUS ONE: `AdminSponsorships` called `revoke.mutate()` straight from its onClick, so a single misplaced click took a supplier's LIVE marketplace placement down immediately, with no statement of the consequence and no way back — §77 requires confirmation proportional to risk and this is an outward-facing, commercially material act. It now names the placement, says what stops appearing and when, says what survives for the audit, and cancel is proven in SQL to revoke nothing. Reverting it kills 6 checks. (4) HYGIENE: `© 2025` is now `new Date().getFullYear()`. Footer links were already audited in an earlier pass (the dead "Company" column withdrawn as an owner decision) and every homepage journey destination is walked in a browser. (5) DOCUMENTATION TRUTH: `MARKETING_AUDIT.md` is preserved as evidence and NOT rewritten; its `branch: main` finding is struck through in place and a banner marks it superseded by the owner-approved RC-tracking `render.yaml`, while the half that still holds — repository configuration is not deployment proof — is restated. THREE PROBE DEFECTS FIXED ALONG THE WAY, each of which had made the sweep lie: `localStorage` on a fresh tab's `about:blank` origin throws a SecurityError and killed the whole run on line one with an uninformative "Uncaught"; the destructive-action check hunted for a "Bulk reject" that only renders once an applicant is SELECTED from a compliance queue that lists applicants who have submitted DOCUMENTS, so it failed the product for the fixture's shape twice before the target was changed; and the Arabic check first scanned whole pages, reporting twenty "untranslated" findings of which nineteen were somebody's NAME. Evidence: `evidence/zg-visualqa-run1.txt` / `-run2.txt` (434/434 twice, identical verdicts), all six sibling probes re-run green, 4730 tests, typecheck and production build green.
