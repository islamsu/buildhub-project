...
- [ ] SAVED → RFQ IS NOT COMPLETE, and must not be reported as complete. The Saved shortlist works — save, list, remove, private by construction, you cannot save what you cannot see — and its CTA navigates to `/rfq`. That is a LINK, not a journey: the buyer arrives at an empty create form and the products and providers they spent two days shortlisting are left behind on the page they came from. §6 asks whether the next part of the product knows it happened, and here it does not. The pieces already exist and are not yet joined: `shared/rfqBasket.ts` (`MAX_BASKET_ITEMS = 30`) carries selected products into the create flow, and `/rfq?invite=<id>` plus `rfq.inviteSupplier` carries a named provider into the invitation. REMAINING: select-on-the-shortlist (some, not all), carry the selected products into the basket and the selected providers into the invite list, and show on arrival WHAT was carried — a basket that silently contains things is its own defect. Until that lands, the Saved → RFQ arc is INCOMPLETE.
