# BuildHub inventory

> Generated by `node scripts/inventory.mjs` from the source itself. Do not edit by hand.
> `server/inventory.test.ts` fails if this file is stale.

## Routes (47)

| Path | Component | Declares `:param` | Reads it |
|---|---|---|---|
| `/` | Home | — | — |
| `/auth` | AuthPage | — | — |
| `/dashboard` | HomeownerDashboard | — | — |
| `/provider` | ProviderDashboard | — | — |
| `/vendor/:id` | VendorProfile | yes | yes |
| `/platform/:role` | RolePlatform | yes | yes |
| `/admin/login` | AdminLogin | — | — |
| `/admin/accept-invitation` | AdminAcceptInvitation | — | — |
| `/admin/admins` | AdminAdmins | — | — |
| `/admin/categories` | AdminCategories | — | — |
| `/admin/users/:id` | AdminUserDetail | yes | yes |
| `/admin/projects/:id` | AdminProjectDetail | yes | yes |
| `/admin/:section/:record` | AdminDashboard | yes | yes |
| `/admin/:section` | AdminDashboard | yes | yes |
| `/admin` | AdminDashboard | — | — |
| `/pricing` | Pricing | — | — |
| `/marketplace` | MarketplaceHub | — | — |
| `/marketplace/products` | Marketplace | — | — |
| `/marketplace/products/:id` | ProductDetail | yes | yes |
| `/marketplace/vendors/:id` | RedirectToVendor | yes | yes |
| `/marketplace/vendors` | VendorsDirectory | — | — |
| `/saved` | SavedPage | — | — |
| `/marketplace/designers/:id` | RedirectToVendor | yes | yes |
| `/marketplace/designers` | DesignersDirectory | — | — |
| `/marketplace/finishing/:id` | RedirectToVendor | yes | yes |
| `/marketplace/finishing` | FinishingDirectory | — | — |
| `/projects/:id` | ProjectDetail | yes | yes |
| `/rfq` | RFQPage | — | — |
| `/rfq/:id/respond` | RFQRespondPage | yes | yes |
| `/rfq/:id` | RFQDetail | yes | yes |
| `/quotations/:id` | QuotationDetail | yes | yes |
| `/messages` | MessagesPage | — | — |
| `/compliance` | CompliancePage | — | — |
| `/auth/setup-password` | PasswordSetupPage | — | — |
| `/auth/reset-password` | PasswordResetPage | — | — |
| `/products/new` | NewProductPage | — | — |
| `/products/:id/edit` | EditProductPage | yes | yes |
| `/settings` | SettingsPage | — | — |
| `/enquiries` | EnquiriesPage | — | — |
| `/disputes/:id` | DisputeDetail | yes | yes |
| `/disputes` | MyDisputes | — | — |
| `/support/:id` | SupportTicketDetail | yes | yes |
| `/support` | MySupport | — | — |
| `/service-categories` | ServiceCategoriesPage | — | — |
| `/catalogue` | CataloguePage | — | — |
| `/ai` | AIAssistantPage | — | — |
| `/404` | NotFound | — | — |

Dead param routes: **0**

## Server procedures (292)

- `protectedProcedure` — 93
- `adminWith:marketplace.manage` — 50
- `publicProcedure` — 35
- `approvedProviderProcedure` — 31
- `adminWith:support.manage` — 20
- `superAdminProcedure` — 10
- `adminWith:users.read` — 9
- `adminWith:billing.manage` — 8
- `adminWith:qa.manage` — 7
- `complianceProcedure` — 6
- `adminWith:users.manage` — 6
- `adminWith:audit.read` — 6
- `adminWith:billing.read` — 4
- `adminProcedure` — 4
- `adminWith:settings.manage` — 2
- `aiChatProcedure` — 1

## Tables (62)

| Table | Columns | FK refs | NOT NULL | UNIQUE |
|---|---|---|---|---|
| `users` | 48 | 2 | 11 | 1 |
| `revokedSessions` | 5 | 1 | 3 | 0 |
| `userAccountAuditEvents` | 9 | 2 | 2 | 0 |
| `adminInvitations` | 13 | 3 | 6 | 1 |
| `projects` | 19 | 2 | 6 | 0 |
| `milestones` | 8 | 1 | 3 | 0 |
| `tasks` | 13 | 3 | 3 | 0 |
| `aiAttachments` | 10 | 1 | 6 | 0 |
| `documents` | 16 | 4 | 5 | 0 |
| `registrationDocuments` | 18 | 2 | 10 | 0 |
| `registrationDocumentSubmissions` | 15 | 1 | 9 | 0 |
| `registrationReviewEvents` | 11 | 3 | 4 | 0 |
| `productQuestions` | 17 | 4 | 4 | 0 |
| `productAnswerRevisions` | 7 | 2 | 4 | 0 |
| `productQuestionReports` | 14 | 3 | 6 | 0 |
| `products` | 31 | 2 | 6 | 0 |
| `portfolioItems` | 12 | 1 | 4 | 0 |
| `rfqs` | 19 | 2 | 6 | 0 |
| `rfqItems` | 13 | 2 | 5 | 0 |
| `quotations` | 18 | 2 | 5 | 0 |
| `messages` | 16 | 4 | 4 | 0 |
| `notifications` | 11 | 1 | 3 | 0 |
| `notificationPreferences` | 7 | 1 | 5 | 0 |
| `serviceOfferings` | 17 | 2 | 7 | 0 |
| `reviews` | 14 | 4 | 5 | 0 |
| `reviewResponses` | 8 | 2 | 5 | 0 |
| `reviewReports` | 13 | 3 | 5 | 0 |
| `progressReports` | 9 | 2 | 6 | 0 |
| `disputes` | 35 | 7 | 11 | 0 |
| `disputeEvidence` | 12 | 3 | 7 | 0 |
| `disputeMessages` | 7 | 2 | 4 | 0 |
| `disputeStatusHistory` | 8 | 2 | 4 | 0 |
| `supportTickets` | 24 | 5 | 8 | 0 |
| `supportTicketMessages` | 8 | 2 | 5 | 0 |
| `supportTicketAttachments` | 11 | 3 | 7 | 0 |
| `supportTicketStatusHistory` | 8 | 2 | 4 | 0 |
| `adminSettings` | 6 | 1 | 4 | 1 |
| `dailyLogs` | 10 | 2 | 5 | 0 |
| `expenses` | 10 | 1 | 3 | 0 |
| `vendorSubscriptions` | 31 | 1 | 10 | 0 |
| `analyticsEvents` | 11 | 1 | 2 | 0 |
| `billingEvents` | 13 | 3 | 2 | 0 |
| `productCategories` | 16 | 2 | 8 | 1 |
| `productCategoryAliases` | 7 | 2 | 4 | 1 |
| `vendorCategories` | 6 | 1 | 3 | 0 |
| `qualifiedEnquiries` | 10 | 2 | 4 | 0 |
| `testLoginTokens` | 11 | 3 | 5 | 1 |
| `commercialAuditEvents` | 12 | 2 | 4 | 0 |
| `vendorEntitlementOverrides` | 15 | 3 | 5 | 0 |
| `fieldValueHistory` | 14 | 2 | 4 | 0 |
| `projectMembers` | 12 | 4 | 5 | 0 |
| `rfqSuppliers` | 14 | 3 | 5 | 0 |
| `vendorProfiles` | 23 | 1 | 3 | 0 |
| `vendorNameChangeRequests` | 15 | 2 | 7 | 0 |
| `adminNotes` | 9 | 1 | 6 | 0 |
| `referrals` | 22 | 3 | 6 | 0 |
| `referralCampaigns` | 21 | 1 | 13 | 0 |
| `referralRewards` | 18 | 3 | 8 | 0 |
| `savedItems` | 8 | 1 | 4 | 0 |
| `referralCodeEvents` | 10 | 2 | 4 | 0 |
| `vendorSponsorships` | 22 | 4 | 7 | 0 |
| `enquiryAssignments` | 9 | 4 | 3 | 0 |

## Notification write sites (32)

- server/routers.ts:774 — notifyUser
- server/routers.ts:1619 — notifyUser
- server/routers.ts:1707 — notifyUser
- server/routers.ts:2980 — notifyUser
- server/routers.ts:3060 — notifyUser
- server/routers.ts:3111 — notifyUser
- server/routers.ts:4064 — notifyUser
- server/routers.ts:4121 — notifyUser
- server/routers.ts:4720 — notifyUser
- server/routers.ts:5105 — notifyUser
- server/routers.ts:5520 — notifyUser
- server/routers.ts:5774 — notifyUser
- server/routers.ts:5966 — notifyUser
- server/routers.ts:6136 — notifyUser
- server/routers.ts:7785 — notifyUser
- server/routers.ts:8128 — notifyUser
- server/routers.ts:8206 — notifyUser
- server/routers.ts:9052 — notifyUser
- server/routers.ts:9123 — notifyUser
- server/routers.ts:9527 — notifyUser
- server/routers.ts:9744 — notifyUser
- server/routers.ts:9774 — notifyUser
- server/routers.ts:9807 — notifyUser
- server/routers.ts:10021 — notifyUser
- server/routers.ts:10068 — notifyUser
- server/routers.ts:10170 — notifyUser
- server/routers.ts:10252 — notifyUser
- server/routers.ts:10373 — notifyUser
- server/notifications.ts:41 — notifyUser
- server/notifications.ts:51 — helper implementation
- server/notifications.ts:65 — notifyUser
- server/notifications.ts:78 — helper implementation

## Storage-dependent surfaces (14)

- `uploadDocument` (complianceProcedure)
- `replaceDocument` (protectedProcedure)
- `archiveDocument` (protectedProcedure)
- `restoreDocument` (protectedProcedure)
- `uploadDocument` (protectedProcedure)
- `uploadProductImage` (approvedProviderProcedure)
- `uploadAttachment` (protectedProcedure)
- `uploadQuotationAttachment` (approvedProviderProcedure)
- `uploadAttachment` (protectedProcedure)
- `uploadImage` (approvedProviderProcedure)
- `uploadAvatar` (protectedProcedure)
- `reviewComplianceDocument` (adminWith:marketplace.manage)
- `uploadAttachment` (protectedProcedure)
- `deleteAttachment` (protectedProcedure)

## Roles

- Workspace roles: `homeowner`, `contractor`, `engineer`, `architect`, `supplier`, `project_manager`
- Additional: `admin`, `anonymous`
